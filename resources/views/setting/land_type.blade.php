...
@section('setting_land_type', 'active')
@section('main_content')
            ...
            <table id="example1" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">{{ __('जग्गा') }}</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($land_types as $key => $land_type)
                        <tr>
                            ...
                            <td class="text-center">{{ $land_type->name }}
                            </td>
                            <td class="text-center"><a class="btn-sm btn-success text-white" data-toggle="modal"
                                ...
                                {{-- modal for adding land_type status --}}
                                <div class="modal fade text-sm" id="modal-lg{{$key+1}}">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="">{{ __('जग्गा सच्याउनुहोस् ') }}</h5>
                                            ...
                                            <div class="modal-body">
                                                <form method="post" action="{{ route('land-type.update',$land_type) }}">
                                                    @method('PUT')
                                                    @csrf
                                                    <div class="row">
                                                        <div class="col-6">
                                                            <div class="input-group input-group-sm">
                                                                ...
                                                                <input type="text" value="{{ $land_type->name }}" name="name"
                                                                    class="form-control  @error('name') is-invalid @enderror">
                                                                @error('name')
                                                                    <p class="invalid-feedback mb-0" style="font-size: 0.9rem">
                                                                        {{ __('जग्गाको फिल्ड खाली छ ') }}
                                                                    </p>
                                                                @enderror
                                                            </div>
                                                        </div>
                                ...
                                {{-- end of modal for adding land_type status --}}
                            </td>
                        </tr>
                    @endforeach
            </table>
    ...
    <div class="modal fade text-sm" id="modal-lg">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="">{{ __('जग्गा थप्नुहोस') }}</h5>
                    <button type=" button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form method="post" action="{{ route('land-type.store') }}">
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <div class="input-group input-group-sm">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">
                                            {{ __('जग्गा') }} <span class="text-danger px-1 font-weight-bold">*</span>
                                        </span>
                                    </div>
